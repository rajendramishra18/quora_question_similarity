...
def tokenize(doc , model):
	...
	for each in doc:
		...
		for sent in text.sents:
			pos = []
			for word in sent:
				pos_dict = {"word" : word , "tag" : word.tag_ , "pos" : word.pos_}
				pos.append(pos_dict)
			pos_doc.append(pos)
		pos_list.append(pos_doc)
	return pos_list

def remove_stopwords(stop, pos):
	for i in range(0 , len(pos)):
		for j in range(0 , len(pos[i])):
			sent_text = []
			for k in range(0 , len(pos[i][j])):
				#print(i , j , k, pos[i][j][k] )
				if pos[i][j][k]["word"] not in stop:
					sent_text.append(pos[i][j][k])
			pos[i][j] = sent_text
...
def is_capital(word):
	return int(not word.isupper() and not word.islower())
...
def contains_special_char(word):
	return int(not word.isalnum() and not word.isdigit() and not word.isalpha())
...
doc_cleaned = remove_stopwords(stop, pos)
